...
# For me 7.15.0 is the latest we palce it in /vagrant to not download it again
...
/usr/share/elasticsearch/bin/elasticsearch-certutil cert --keep-ca-key ca --pem --in /tmp/certs/instance.yml --out /tmp/certs/certs.zip
...
/usr/share/elasticsearch/bin/elasticsearch-setup-passwords auto -b > /root/ESUsers.txt

# Add the Kibana password to the keystore
grep "kibana_system =" /root/ESUsers.txt | awk '{print $4}' | sudo /usr/share/kibana/bin/kibana-keystore add --stdin elasticsearch.password
...
echo "To get the elastic password ssh into the box and issue"
echo "sudo grep 'elastic =' /root/ESUsers.txt | awk '{print $4}'"